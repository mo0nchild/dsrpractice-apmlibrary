...
{
    public partial class UpdateBookViewModel : IMappingWith<UpdateBookCommand>, IValidatableObject
    {
        ...
        {
            profile.CreateMap<UpdateBookViewModel, UpdateBookCommand>()
            ...
            profile.CreateMap<GenreDto, string>().ConvertUsing((value, dest) => value.Name);

            profile.CreateMap<BookDto, UpdateBookViewModel>()
                .ForMember(item => item.BookId, options => options.MapFrom(item => item.Id))
                .ForMember(item => item.Title, options => options.MapFrom(item => item.Title))
                .ForMember(item => item.AuthorName, options => options.MapFrom(item => item.AuthorName))

                .ForMember(item => item.YearIssue, options => options.MapFrom(item => item.YearIssue.ToDateTime(TimeOnly.MinValue)))
                .ForMember(item => item.PublishType, options => options.MapFrom(item => item.PublicationType))
                .ForMember(item => item.Language, options => options.MapFrom(item => item.Language))
